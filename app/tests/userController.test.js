...
describe('UserController Test Suite', () => {
    test('get should return an array of users', async () => {
        let response = await request(app).get(URL_USERS);
    ...
    test('post should return saved id', async () => {
        ...
        let response = await request(app).post(URL_USERS).send(user);
        expect(response.statusCode).toBe(201);
        let body = response.body;
        expect(body.length).toBe(24);
        let savedUserResponse = await request(app).get(URL_USERS + '/' + body);
    ...
    test('get by id should return an user', async () => {
        let response = await request(app).get(URL_USERS + '/1');
    ...
    test('put should update an existing user', async () => {
        ...
        let response = await request(app).put(URL_USERS).send(user);
        expect(response.statusCode).toBe(200);
        let updatedUserResponse = await request(app).get(URL_USERS + '/1');
    ...
    test('delete by id should return success message', async () => {
        let response = await request(app).delete(URL_USERS + '/1');
        expect(response.statusCode).toBe(200);
        let deletedUserResponse = await request(app).get(URL_USERS + '/1');
